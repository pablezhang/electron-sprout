...
export default class PlayContentCard extends Component<any, any>{
  ...
  componentDidUpdate(props, state) {
    if (this.props.playcontent.currentLyric != props.playcontent.currentLyric) {
      console.log('change');
// @ts-ignore
      let target = this.refs.current as any;
// @ts-ignore
      let container = this.refs.lyric as any;
      this.setState({
  ...
  _starSong(e, id) {
    ...
    playlistTracks('add', pid, id).then((res:any) => {
      if (res.code == 502) {
